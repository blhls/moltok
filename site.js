/* ============================================================
   ...
   Moltok · rail clock · feed search · demiurge flip · moult confirm.
...
function flipDemiBack(btn) {
  var card = btn.closest('.demi-card');
  if (card) card.classList.remove('flipped');
}

/* ── Transmission reveal (clock / berylism banner / birt) ─
   Hidden until you click the crab button — that's the point. */
function toggleTransmission(btn) {
  var extra = document.getElementById('rail-extra');
  if (!extra) return;
...
/* ── Moult confirmation ──────────────────────────────────── */
function openMoultConfirm() {
  var p = document.getElementById('moult-popup');
  if (p) p.hidden = false;
}
function closeMoultConfirm() {
  var p = document.getElementById('moult-popup');
  if (p) p.hidden = true;
}
function confirmMoultOutside(e) {
  if (e.target.id === 'moult-popup') closeMoultConfirm();
}
function confirmMoult() {
  window.location.href = document.body.dataset.root + 'index.html';
}
